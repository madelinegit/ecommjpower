...
// 'available' is deliberately the default: an item is for sale unless said
// otherwise, so forgetting the field never hides a piece.
const AVAILABILITY = ['available', 'made_to_order', 'sold'];
const KINDS = ['photo', 'before', 'after'];
const clean = v => (typeof v === 'string' ? v.trim() : '');

function normalizeKind(value) {
  const k = clean(value).toLowerCase();
  return KINDS.includes(k) ? k : 'photo';
}

function normalizeAvailability(value) {
  const a = clean(value).toLowerCase();
  return AVAILABILITY.includes(a) ? a : 'available';
}
...
async function attachImages(items) {
  ...
  const { rows } = await db.query(
    `SELECT id, item_id, url, thumb_url, width, height, position, kind
  ...
  items.forEach(i => {
    ...
    i.images = all.filter(im => im.kind === 'photo');
    i.before = all.find(im => im.kind === 'before') || null;
    i.after  = all.find(im => im.kind === 'after')  || null;
    // A slider needs both halves; one alone is just a photo.
    i.hasSlider = Boolean(i.before && i.after);
    // Something has to render in the grid even if only a pair was uploaded.
    i.cover = i.images[0] || i.after || i.before || null;
  });
  return items;
}
  ...
  try {
    const item = await db.withTransaction(async client => {
      const { rows } = await client.query(
        `INSERT INTO gallery_items (title, description, features, price, category, availability, position)
         VALUES ($1, $2, $3, $4, $5, $6,
                 COALESCE((SELECT MAX(position) FROM gallery_items WHERE category = $5), 0) + 1)
         RETURNING *`,
        [title, description, clean(req.body.features) || null, price, category,
         normalizeAvailability(req.body.availability)]
      );
    ...
    const updated = await db.withTransaction(async client => {
      ...
      const { rows } = await client.query(
        `UPDATE gallery_items
            SET title = $1, description = $2, features = $3, price = $4, category = $5,
                availability = $6,
                featured = CASE WHEN $7 THEN FALSE ELSE featured END,
                position = CASE WHEN $7
                  THEN COALESCE((SELECT MAX(position) FROM gallery_items WHERE category = $5), 0) + 1
                  ELSE position END,
                updated_at = NOW()
          WHERE id = $8
        RETURNING *`,
        [title, clean(req.body.description) || null, clean(req.body.features) || null,
         clean(req.body.price) || null, newCategory,
         normalizeAvailability(req.body.availability), moved, id]
      );
  ...
  try {
    ...
    for (const file of req.files || []) {
      try {
        ...
        // A piece has at most one before and one after, so uploading a
        // replacement clears the old one rather than stacking another.
        if (kind !== 'photo') {
          const { rows: old } = await db.query(
            `DELETE FROM gallery_images WHERE item_id = $1 AND kind = $2
             RETURNING storage_key, thumb_key`,
            [id, kind]
          );
          for (const o of old) {
            await storage.deleteObject(o.storage_key);
            await storage.deleteObject(o.thumb_key);
          }
        }

        const { rows } = await db.query(
          `INSERT INTO gallery_images
             (item_id, url, thumb_url, storage_key, thumb_key, width, height, kind, position)
           VALUES ($1,$2,$3,$4,$5,$6,$7,$8,
                   COALESCE((SELECT MAX(position) FROM gallery_images WHERE item_id = $1), -1) + 1)
           RETURNING *`,
          [id, img.url, img.thumbUrl, img.storageKey, img.thumbKey, img.width, img.height, kind]
        );
        stored.push(rows[0]);
      } catch (err) {
        failures.push(err instanceof storage.UploadError
